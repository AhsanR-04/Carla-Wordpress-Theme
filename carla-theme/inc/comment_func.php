...
add_action('wp_enqueue_scripts', 'carla_reply');


/**
 * Comments
 */
if (!function_exists('carla_comment')):
  /**
   * Template for comments and pingbacks.
   *
   * Used as a callback by wp_list_comments() for displaying the comments.
   */
  function carla_comment($comment, $args, $depth)
  {
    ...
    if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type): ?>
      ...
      <div id="comment-<?php comment_ID(); ?>" <?php comment_class('media alert alert-info'); ?>>
        <div class="comment-body">
          <?php _e('Pingback:', 'carla'); ?>
          <?php comment_author_link(); ?>
          <?php edit_comment_link(__('Edit', 'carla'), '<span class="edit-link">', '</span>'); ?>
        </div>
            ...
            <div class="comment-content w-100">
                  ...
                  <div class="user-info d-flex flex-wrap justify-content-between align-items-center">
                    <div class="mt-0 text-capitalize">
                      <?php printf(__('%s <span class="says d-none">says:</span>', 'carla'), sprintf('<p class="h5">%s</p>', get_comment_author())); ?>
                    </div>

                    <div class="comment-meta d-flex align-items-center flex-wrap column-gap-3">
                      <div class="comment-info">
                        <time datetime="<?php comment_time('c'); ?>">
                          <?php printf(_x('%1$s at %2$s', '1: date, 2: time', 'carla'), get_comment_date('j M Y'), get_comment_time()); ?>
                        </time>
                        <?php edit_comment_link(__('Edit', 'carla'), '<span class="edit-link">', '</span>'); ?>
                  ...
                  <?php if ('0' == $comment->comment_approved): ?>
                    <p class="comment-awaiting-moderation alert alert-info">
                      <?php _e('Your comment is awaiting moderation.', 'carla'); ?>
                    </p>
                  <?php endif; ?>
...
function wp44138_change_comment_form_cookies_consent($fields)
{
  ...
  $fields['cookies'] = '<p class="comment-form-cookies-consent custom-control form-check mb-3">' .
    ...
    '<label for="wp-comment-cookies-consent" class="form-check-label">' . __('Save my name, email, and website in this browser for the next time I comment.', 'carla') . '</label>' .
    '</p>';
 ...
 * Comment Button
 */
if (!function_exists('carla_comment_button')):
  function carla_comment_button($args)
  {
  ...
  add_filter('comment_form_defaults', 'carla_comment_button');
endif;
